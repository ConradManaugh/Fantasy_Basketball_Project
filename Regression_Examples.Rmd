...
labs(title = paste0("Predicted Field Goal's of ",Name[6*ii-5]),
...
```{r random package}
install.packages("sqldf")##The internet suggested i use this pacckage
library(sqldf)
```
...
Name_Levels = sqldf("select DISTINCT Name as 'Name' from DatSet")
tail(Name_Levels,25)#Now I know all the name levels, I have also confirmed their are no repeats so I no longer need to use this package or function anymore.
```
```{r graphs02}
GraphFG(4)
```
