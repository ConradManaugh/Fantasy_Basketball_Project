...
predAge=rep(NA,5*length(All_Season_Stats$Player))##This will end up being a variable of the age that he will be predicing
...
predAge[k]=All_Season_Stats$Age[floor(k/5)+1]+1##The max age plus one is the predicted age
...
In_Arc[l+1]=All_Season_Stats$`2P_17`[floor(l/5)+1]###Every 5th entry is the 2 points made for 18, and so on
...
FreeT[ll+1]=All_Season_Stats$FT_17[floor(ll/5)+1]###Every 5th entry is the free throw made for 18, and so on
...
w=1
while(w<=(5*length(All_Season_Stats$Player))){
OfReb[w]=All_Season_Stats$ORB_18[floor(w/5)+1]
OfReb[w+1]=All_Season_Stats$ORB_17[floor(w/5)+1]###Every 5th entry is the offreb for 18, and so on
OfReb[w+2]=All_Season_Stats$ORB_16[floor(w/5)+1]
OfReb[w+3]=All_Season_Stats$ORB_15[floor(w/5)+1]
OfReb[w+4]=All_Season_Stats$ORB_14[floor(w/5)+1]
...
ww=1
while(ww<=(5*length(All_Season_Stats$Player))){
DefReb[ww]=All_Season_Stats$DRB_18[floor(ww/5)+1]
DefReb[ww+1]=All_Season_Stats$DRB_17[floor(ww/5)+1]###Every 5th entry is the defreb for 18, and so on
DefReb[ww+2]=All_Season_Stats$DRB_16[floor(ww/5)+1]
DefReb[ww+3]=All_Season_Stats$DRB_15[floor(ww/5)+1]
DefReb[ww+4]=All_Season_Stats$DRB_14[floor(ww/5)+1]
...
m=1
while(m<=(5*length(All_Season_Stats$Player))){
Assist[m]=All_Season_Stats$AST_18[floor(m/5)+1]
Assist[m+1]=All_Season_Stats$AST_17[floor(m/5)+1]###Every 5th entry is the assist for 18, and so on
Assist[m+2]=All_Season_Stats$AST_16[floor(m/5)+1]
Assist[m+3]=All_Season_Stats$AST_15[floor(m/5)+1]
Assist[m+4]=All_Season_Stats$AST_14[floor(m/5)+1]
...
mm=1
while(mm<=(5*length(All_Season_Stats$Player))){
Steal[mm]=All_Season_Stats$STL_18[floor(mm/5)+1]
Steal[mm+1]=All_Season_Stats$STL_17[floor(mm/5)+1]###Every 5th entry is the steal for 18, and so on
Steal[mm+2]=All_Season_Stats$STL_16[floor(mm/5)+1]
Steal[mm+3]=All_Season_Stats$STL_15[floor(mm/5)+1]
Steal[mm+4]=All_Season_Stats$STL_14[floor(mm/5)+1]
...
q=1
while(q<=(5*length(All_Season_Stats$Player))){
Block[q]=All_Season_Stats$BLK_18[floor(q/5)+1]
Block[q+1]=All_Season_Stats$BLK_17[floor(q/5)+1]###Every 5th entry is the block for 18, and so on
Block[q+2]=All_Season_Stats$BLK_16[floor(q/5)+1]
Block[q+3]=All_Season_Stats$BLK_15[floor(q/5)+1]
Block[q+4]=All_Season_Stats$BLK_14[floor(q/5)+1]
...
qq=1
while(qq<=(5*length(All_Season_Stats$Player))){
TOV[qq]=All_Season_Stats$TOV_18[floor(qq/5)+1]
TOV[qq+1]=All_Season_Stats$TOV_17[floor(qq/5)+1]###Every 5th entry is the turnover for 18, and so on
TOV[qq+2]=All_Season_Stats$TOV_16[floor(qq/5)+1]
TOV[qq+3]=All_Season_Stats$TOV_15[floor(qq/5)+1]
TOV[qq+4]=All_Season_Stats$TOV_14[floor(qq/5)+1]
...
s=1
while(s<=(5*length(All_Season_Stats$Player))){
Fouls[s]=All_Season_Stats$PF_18[floor(s/5)+1]
Fouls[s+1]=All_Season_Stats$PF_17[floor(s/5)+1]###Every 5th entry is the turnover for 18, and so on
Fouls[s+2]=All_Season_Stats$PF_16[floor(s/5)+1]
Fouls[s+3]=All_Season_Stats$PF_15[floor(s/5)+1]
Fouls[s+4]=All_Season_Stats$PF_14[floor(s/5)+1]
...
```{r the data set}
DatSet=data.frame(Name,Pos2,Age,predAge,Year,FG,Bhind_Arc,In_Arc,FreeT,OfReb,DefReb,Assist,Steal,Block,TOV,Fouls)
tail(DatSet,25)
...
```{r function Field Goals}
Fun_FG=function(Age,Name){##Function takes vector of names and ages contained in DatSet
  
  
  
  Age=(which(DatSet)==Name)##ages we care about are the ones when that name played
  Age=c(Age,max(Age)+1)##we want to predict his next year so we find his oldest age and add one
  if(Name=="Aaron Brooks"){##Aaron brooks is default so coef is 0
    coef=0
  }else{
  

    coef=as.numeric(fit1$coefficients[(which(All_Season_Stats==Name))+2])##the corresponding coefficient for each player is how that player changes the graph, with 3 non name coeffiecents and one default we only have to add 2 to shift it correctly
  }
  pred=-1.900e+03+(-3.069e+00*Age*Age)+162.52126*Age+coef##The other coeffiecents don't change
